...
model = load_model('main/Sentiment_LSTM_model.h5', compile = False)
...
def analyzehashtag():
    ...
    neutral = 0
    negative = 0
    for tweet in tweepy.Cursor(api.search_tweets,q="#" + request.args.get("text") + " -filter:retweets",lang="en", tweet_mode='extended').items(100):
        with graph.as_default():
...
def gettweets():
    tweets = []
    for tweet in tweepy.Cursor(api.search_tweets,q=request.args.get("text") + " -filter:retweets",lang="en", tweet_mode='extended').items(15):
        temp = {}
